...
项目当前处于 **MVP（0.1.0）** 阶段。Windows 英文键盘和微信输入法的中英文模式已经通过原型验证；其他输入法仍需逐项实测，暂不作兼容承诺。
...
- 目前仅验证 Windows 英文键盘与微信输入法。微软拼音、搜狗、百度等需在真实环境中补充兼容性测试。
...
3. 创建并推送版本标签：`git tag v0.1.0`、`git push origin v0.1.0`。
