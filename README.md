...
5. 右键托盘图标，可以查看状态、暂停/继续着色、设置开机自动启动或退出程序。
...
其他输入法仍可能存在实现差异，欢迎通过 GitHub Issues 反馈兼容性结果。
...
推送分支、Pull Request 或 `v*` 标签时，GitHub Actions 会在 Windows 环境构建、运行核心测试、按配置签名并上传 ZIP 与 SHA-256 校验文件。正式 GitHub Release 由维护者检查构建包后发布。
